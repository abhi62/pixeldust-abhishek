                   ...
                   return (
                       <>
                        <div className="header">
                            {key[0]}
                            <span>{key[1].length} shifts, {getHours(key[1])[0]} h {getHours(key[1])[1]} m</span>
                        ...
                        {sortByTime(key[1]).map(row => {
                            return(
                                <div className="table-r">
                                    ...
                                    <button className="btn-cancel" onClick={(e) => ShiftCancel(e, row.id)} disabled={(new Date() >= new Date(row.startTime) && new Date() <= new Date(row.endTime)) ? true : false}>Cancel</button>
                                </div>
                            )
                        })}
